        ...
        <div className="max-w-6xl mx-auto p-6 md:p-10 pb-24 text-black">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 text-black">
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm text-black">
              <div className="flex items-center justify-between mb-4"><span className="p-2 bg-slate-100 rounded-lg text-slate-600"><FileBox className="w-5 h-5" /></span><span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total</span></div>
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm border-t-4 border-t-emerald-500">
              <div className="flex items-center justify-between mb-4 text-black"><span className="p-2 bg-emerald-50 rounded-lg text-emerald-600"><CheckCircle2 className="w-5 h-5" /></span><span className="text-xs font-bold text-emerald-500 uppercase tracking-wider">Active</span></div>
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm border-t-4 border-t-rose-500">
              <div className="flex items-center justify-between mb-4 text-black"><span className="p-2 bg-rose-50 rounded-lg text-rose-600"><AlertCircle className="w-5 h-5" /></span><span className="text-xs font-bold text-rose-500 uppercase tracking-wider">Issues</span></div>
              <p className="text-2xl font-bold text-slate-900">{stats.failed} Failed</p>
            </div>
          </div>
          ...
          {files.some(f => f.status === 'indexing') && (
            <div className="mb-10 animate-in fade-in slide-in-from-top-4 duration-500 text-black">
              <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl overflow-hidden">
                ...
                <div className="p-6 h-64 overflow-y-auto font-mono text-xs flex flex-col-reverse custom-scrollbar bg-slate-950">
                  <div className="space-y-1">
                    {files.filter(f => f.status === 'indexing').map(f => (
                      <div key={f.id} className="mb-6 border-l-2 border-indigo-500/30 pl-4">
                        <p className="text-emerald-400 font-bold mb-2 flex items-center gap-2 text-black">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 text-black">
            <div className="lg:col-span-4 text-black">
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm lg:sticky lg:top-6 text-black">
                <h2 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2 text-black"><UploadCloud className="w-5 h-5 text-[#D32F2F]" /> Upload Source</h2>
                <form onSubmit={handleUpload} className="space-y-6">
                  <div className={`relative group border-2 border-dashed rounded-xl p-8 transition-all text-center ${selectedFile ? 'border-[#D32F2F] bg-red-50/30' : 'border-slate-200 hover:border-red-400 bg-slate-50/50'}`}>
                    ...
                    <div className="flex flex-col items-center gap-3">
                      {selectedFile ? (
                        <><div className="w-12 h-12 bg-[#D32F2F] text-white rounded-xl flex items-center justify-center shadow-lg shadow-red-200"><FileText className="w-6 h-6" /></div><div className="text-sm font-semibold text-slate-700 truncate max-w-full px-4 text-black">{selectedFile.name}</div><button type="button" onClick={(e) => { e.stopPropagation(); setSelectedFile(null); }} className="text-xs font-bold text-rose-500 hover:text-rose-600 text-black">Remove</button></>
                      ) : (
                        <><div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-slate-400 border border-slate-200 shadow-sm group-hover:text-[#D32F2F] transition-all text-black"><Plus className="w-6 h-6" /></div><div className="space-y-1 text-black"><p className="text-sm font-bold text-slate-700 text-black">Drop file here</p><p className="text-xs text-slate-500 font-medium">PDF, Markdown or Text</p></div></>
                      )}
                    </div>
                  </div>
                  ...
                  <button type="submit" disabled={!selectedFile || uploading || !mode || files.some(f => f.status === 'indexing') || !supabase} className={`w-full py-3.5 rounded-xl font-bold transition-all flex items-center justify-center gap-2 ${!selectedFile || uploading || !mode || files.some(f => f.status === 'indexing') || !supabase ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-[#D32F2F] text-white hover:bg-red-700 active:scale-[0.98] shadow-lg shadow-red-200'}`}>
            ...
            <div className="lg:col-span-8 space-y-8 text-black">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col text-black">
                ...
                <div className="overflow-x-auto text-black">
                  <table className="w-full text-left border-collapse text-black">
                    <thead><tr className="text-[11px] font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100"><th className="px-6 py-4">Name</th><th className="px-6 py-4">Qdrant ID</th><th className="px-6 py-4 text-right">Actions</th></tr></thead>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col text-black">
                <div className="p-4 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between gap-4 text-black"><div className="relative flex-1 text-black"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input type="text" placeholder="Filter files..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white border border-slate-200 rounded-lg py-2 pl-10 pr-4 outline-none focus:ring-2 focus:ring-red-500/20 text-sm font-medium text-black text-black" /></div></div>
                <div className="overflow-x-auto text-black">
                  <table className="w-full text-left border-collapse text-black">
                    <thead><tr className="text-[11px] font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100 text-black"><th className="px-6 py-4 text-black">Document</th><th className="px-6 py-4 text-center">Status</th><th className="px-6 py-4">Date</th><th className="px-6 py-4 text-right">Actions</th></tr></thead>
                    <tbody className="divide-y divide-slate-50 text-black">
                      {filteredFiles.length === 0 ? (<tr><td colSpan={4} className="py-24 text-center text-slate-300 font-semibold text-sm">No documents found</td></tr>) : filteredFiles.map((file) => (
                        <tr key={file.id} className="group hover:bg-slate-50/80 transition-colors text-black"><td className="px-6 py-4 flex items-center gap-3"><div className={`w-10 h-10 rounded-lg flex items-center justify-center border ${file.filename.endsWith('.pdf') ? 'bg-rose-50 text-rose-500 border-rose-100' : 'bg-red-50 text-[#D32F2F] border-red-100'}`}><FileText className="w-5 h-5" /></div><div><p className="text-sm font-bold text-slate-800 line-clamp-1">{file.filename}</p><p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{file.mode}</p></div></td><td className="px-6 py-4 text-center"><span className={`px-3 py-1 rounded-full text-[10px] font-extrabold tracking-tight ${file.status === 'completed' ? 'bg-emerald-100 text-emerald-700' : file.status === 'failed' ? 'bg-rose-100 text-rose-700' : 'bg-red-100 text-red-700 animate-pulse'}`}>{file.status.toUpperCase()}</span></td><td className="px-6 py-4 text-xs font-semibold text-slate-500 whitespace-nowrap">{file.createdAt ? new Date(file.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</td><td className="px-6 py-4 text-right text-black"><button onClick={() => handleDelete(file.id)} className="p-2 rounded-lg text-slate-300 hover:text-rose-500 hover:bg-rose-50 transition-all opacity-0 group-hover:opacity-100 text-black text-black"><Trash2 className="w-5 h-5" /></button></td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
      ...
      {showColModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 max-w-md w-full p-8 overflow-hidden relative animate-in fade-in zoom-in duration-200">
            <h3 className="text-xl font-bold text-slate-900 mb-2">Create New Collection</h3>
            <p className="text-slate-500 text-sm mb-6">Tạo một không gian kiến thức mới cho RAG Agent</p>
            <form onSubmit={handleCreateCollection} className="space-y-4">
              <div className="space-y-1.5"><label className="text-xs font-bold uppercase tracking-wider text-slate-400">Display Name</label><input type="text" required placeholder="e.g. Tuyển sinh 2026" value={newColName} onChange={(e) => setNewColName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-red-500/20 focus:border-[#D32F2F] transition-all text-black" /></div>
              <div className="space-y-1.5"><label className="text-xs font-bold uppercase tracking-wider text-slate-400">Qdrant Identifier</label><input type="text" placeholder="vmg_docs_custom" value={newColQName} onChange={(e) => setNewColQName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-red-500/20 focus:border-[#D32F2F] transition-all text-black" /></div>
              <div className="space-y-1.5"><label className="text-xs font-bold uppercase tracking-wider text-slate-400">Description</label><textarea placeholder="e.g. Thông tin về các chương trình du học hè 2026" value={newColDesc} onChange={(e) => setNewColDesc(e.target.value)} rows={3} className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-red-500/20 focus:border-[#D32F2F] transition-all resize-none text-sm text-black" /></div>
              <div className="flex items-center gap-3 mt-8">
                <button type="button" onClick={() => setShowColModal(false)} className="flex-1 py-3 text-sm font-bold text-slate-500 hover:bg-slate-50 rounded-xl transition-all">Hủy bỏ</button>
